...
sms-v4 version \`sms-v4-mcp\` connected.
...
if [[ -d "/mnt/c" ]]; then
  ...
  cp "$SCRIPT_DIR/scripts/idle-detect.ps1" "$CLAUDE_DIR/" 2>/dev/null
  ...
  echo -e "      $CLAUDE_DIR/idle-detect.ps1    (Claude Code: 6-sample idle detection)"
