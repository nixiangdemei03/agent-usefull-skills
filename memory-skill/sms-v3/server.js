#!/usr/bin/env node
/**
 * SMS v2 MCP Server — Smart Memory System × Claude Code Bridge
 *
 * v2 新特性:
...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
  const resources = [
    { uri: "sms://index", name: "Memory Index (JSON)", description: "Master index", mimeType: "application/json" },
    { uri: "sms://cache", name: "Memory Cache", description: "Hot tier + score summary", mimeType: "application/json" },
    { uri: "sms://schema", name: "Memory Schema v2", description: "SMS v2 schema with fingerprint, score, tier", mimeType: "application/json" },
    { uri: "sms://skill", name: "Memory Skill (SMS v2)", description: "SMS v2 behavior guidelines", mimeType: "text/markdown" },
    { uri: "sms://hot", name: "Hot Tier Entries", description: "Top 20 scored entries auto-loaded in sessions", mimeType: "application/json" },
    { uri: "sms://consolidated", name: "Consolidated Memory (Prioritized)", description: "🥇 最高优先级记忆 —— 所有去重合并后按score排序的条目", mimeType: "application/json" },
    { uri: "sms://stats", name: "Memory Stats", description: "Tier distribution, total entries, score range", mimeType: "application/json" },
  ];

  const autoFiles = listJSONFiles("auto");
  for (const f of autoFiles.slice(-7)) {
    const name = f.replace(".json","");
    resources.push({ uri: `sms://auto/${name}`, name: `Auto entries: ${name}`, mimeType: "application/json" });
  }
...
server.setRequestHandler(ListResourceTemplatesRequestSchema, async () => ({
  resourceTemplates: [
    { uriTemplate: "sms://auto/{date}", name: "Auto entries by date", mimeType: "application/json" },
    { uriTemplate: "sms://curated/{id}", name: "Curated entry by id", mimeType: "application/json" },
  ],
}));
...
server.setRequestHandler(ReadResourceRequestSchema, async (req) => {
  ...
  if (uri === "sms://index") return send(readJSON("index.json") || {});
  if (uri === "sms://cache") return send(readJSON("cache/memory_cache.json") || {});
  if (uri === "sms://schema") return send(readJSON("schema.json") || {});
  if (uri === "sms://skill") {
...
async function main() {
  console.error(`[SMS-v2] Starting server`);
  console.error(`[SMS-v2] MEMORY_DIR = ${MEMORY_DIR}`);
  console.error(`[SMS-v2] Exists: ${existsSync(MEMORY_DIR)}`);

  if (!existsSync(MEMORY_DIR)) {
    console.error(`[SMS-v2] ⚠️  Memory directory not found`);
  }

  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error(`[SMS-v2] Server ready ✓`);
}

main().catch((err) => {
  console.error(`[SMS-v2] Fatal:`, err);
  process.exit(1);
});
