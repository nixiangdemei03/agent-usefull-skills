...
echo "[SMS-v2] ╔══════════════════════════════════════╗"
echo "[SMS-v2] ║   Consolidation Start                ║"
echo "[SMS-v2] ╚══════════════════════════════════════╝"
echo "[SMS-v2] Author: $AUTHOR"
echo "[SMS-v2] Date: $(date)"
echo ""

# ── Step 0: Git pull (if enabled) ────────────────────────────
if $GIT_PULL; then
  if [ -d "$MEMORY_DIR/.git" ]; then
    echo "[SMS-v2] ── Git pull ──"
    git -C "$MEMORY_DIR" pull --rebase 2>&1 | sed 's/^/  /'
    echo ""
  elif [ -n "$GIT_REPO" ]; then
    echo "[SMS-v2] ── Git clone ──"
    ...
    echo "[SMS-v2] ⚠️  --git-pull set but no .git dir and no repo URL given"
    echo "[SMS-v2]    Set SMS_GIT_REPO env or pass URL: --git-pull https://..."
    echo ""
  fi
fi

# ── Step 1: Score all entries ────────────────────────────────
echo "[SMS-v2] ── Scoring entries ──"
...
# ── Step 3: Hot / Warm cap ──────────────────────────────────
echo "[SMS-v2] ── Tier cap (Hot≤20, Warm≤100) ──"
